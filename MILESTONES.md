...
- [ ] **M3.1 — Remove `Deref<Target = AsyncDevice>` from sync wrapper**
...
- [ ] **M3.2 — Hide pyo3-only types behind a `#[cfg]` re-export module**
...
- [ ] **M3.3 — Scanner: make `pub` fields private; reconcile static vs builder APIs**
...
- [ ] **M3.5 — `Device::new` should not spawn until connected**
...
- [ ] **M3.6 — `bridge_to_sync` must surface listener errors**
...
- [ ] **M3.7 — `pub fn discover_device_internal` should be crate-private**
  - Site: [scanner.rs:584](src/scanner.rs#L584)
  - Fix: change to `pub(crate)`.

- [ ] **M3.8 — `Device::address`, `dev_type` etc. should not take a `RwLock`**
  - Site: [device.rs:372-407](src/device.rs#L372-L407)
  - Problem: hot-path getters touch a `RwLock` to return cheap values.
  - Fix: store via `ArcSwap<str>` / `AtomicU8` where mutation is rare-but-allowed; keep `RwLock` only for the genuinely-coupled state group.

---

## M4 — Code Hygiene & Refactoring

- [ ] **M4.1 — Extract response-matching logic from `process_command`**
...
- [ ] **M4.2 — Collapse duplicated `generate_payload` arms in v3.1/v3.2/v3.3 and v3.4/v3.5**
...
- [ ] **M5.1 — Trait-ify external dependencies for injection**
  - `trait ScannerHandle` so `Device` can be unit-tested without the global singleton ([device.rs:14, 1037, 1271](src/device.rs#L14)).
  - `trait Clock { fn now(&self) -> Instant; }` to remove `SystemTime::now()` calls ([device.rs:689-694](src/device.rs#L689)).
  - Generic `AsyncRead + AsyncWrite + Unpin` plumbing all the way down to `connect_and_handshake` ([device.rs:1172](src/device.rs#L1172)).
...
- [x] **M5.2 — Integration tests for the actor** *(partial)*
  ...
  - **Still pending**: in-process mock TCP server replaying captured device traffic (v3.3 + CRC, v3.4 + HMAC, v3.5 + GCM) for handshake / session-key negotiation / dev22 fallback / broadcast-lag / error-cmd-0 paths. Requires more setup; left for a follow-up.
...
- [ ] **M5.4 — Scanner integration tests**
  - `set_ports` after start delivers packets on the new port (regression for M1.2).
  - `stop_passive_listener` followed by `scan()` succeeds (regression for M1.1).
  - Concurrent `scan_stream` callers do not both spawn discovery loops (regression for M2.6).

- [ ] **M5.5 — Property-based tests for `ConnectionState` transitions**
  - Use `proptest` to drive `with_state_mut` sequences and assert invariants (e.g. `Stopped` is terminal).

---

## M6 — Documentation & Release Polish

- [ ] **M6.1 — Doc-warning on every sync method** about runtime-context behavior.
- [ ] **M6.2 — Add module-level "when to use sync vs async" doc** to [lib.rs](src/lib.rs).
...
- [ ] **M6.4 — Tracing migration** — replace `log` with `tracing` for span-aware diagnostics in actor loops.
- [ ] **M6.5 — `CHANGELOG.md`** generated via `cliff.toml` (already configured) — include security/correctness items from M1 prominently.
- [ ] **M6.6 — `cargo deny` / `cargo audit`** in CI to track supply-chain.
