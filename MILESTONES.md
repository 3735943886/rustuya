...
- [x] **docs/getting-started.md**: bump `rustuya = "0.2"` (line 13) → `"0.3"` once 0.3.0 stable is tagged. ✅ Flipped to `"0.3"` in the `release/0.3.0` prep. (Cargo will not resolve pre-release versions from a plain `"0.3"` requirement, so it was intentionally left at `"0.2"` during the rc cycle.)
