...
| R4 | Driver `known` map never evicted while the core cache does — unbounded growth, and `find` handing out addresses the core had already forgotten. | Bounded, not time-filtered. The driver map carries a last-seen `Instant` and is capped at `max_devices`: a *new* id at capacity evicts the longest-silent entry (a live device re-announces and stays fresh, so what goes is the departed or the forged). Reads (`find`/`known`) deliberately apply **no** freshness policy — that is P3: `last_seen(id)` exposes the raw age and the caller decides — so an entry for a departed device is served until evicted, and a connect to it just fails and recovers. (An earlier revision of this row promised a `cache_ttl` read filter and a prune-on-`Found`; the code never did that, by the P3 decision above, and the row now says what the code does.) |
| R5 | Announcements are **unauthenticated** — the UDP keys are public constants — yet an announcement's self-reported `ip` is where a linked device's connection is redirected (P3 rewake), and its `id` keys two maps. Any host on the LAN could forge one: relocate a device (a v3.1/3.3 client then sends its encrypted control frames to the attacker; v3.4/3.5 cannot complete the handshake without the local key, so it is a redial-loop DoS), or pin unbounded memory with unique ids. | Trust is an explicit, injected policy in the core `Config`. **`require_source_match`** (driver default **on**): an announcement whose `ip` differs from its datagram's source address is dropped and counted (`Discovery::rejected`; the driver logs the first at `warn`, then `debug`) — a real device announces from its own address. Turn it off only for a topology that relays announcements. **`max_entries`** (driver `max_devices`, default 16 384) bounds the core cache: a new id past the cap is dropped, known ids keep updating. Always on, no knob: ids must be 1–64 printable ASCII bytes, `productKey` ≤ 64, and `ip` must be unicast IPv4 (the LAN protocol is IPv4-only; unspecified/multicast/broadcast/IPv6 are not devices). Pinned by core unit tests and `tests/discovery_spoof.rs` (an order-deterministic E2E: forged then honest announcement from one socket; the honest one must be the first `Found`). |
